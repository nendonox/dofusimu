...
function displayCharacterStatus(type){
    var row = getCharacterStatusRow(type);
    var scroll = $(row.find("#character_status_scroll")[0]);
    var point = $(row.find("#character_status_point")[0]);
    var total = $(row.find("#character_status_total")[0]);

    scroll.html(character["Status"][type]["Scroll"]);
    point.html(character["Status"][type]["Point"]);
    total.html(character["Status"][type]["Total"]);
}
